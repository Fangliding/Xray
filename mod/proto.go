...
func InjectDataToProto[T any](proto any, fieldName string, data T) {
	...
	b := *unkFields
	*unkFields = make(protoimpl.UnknownFields, 0, len(b)+len(encodedData)+10)
	for len(b) > 0 {
		...
		if num != injectDataFieldID {
			*unkFields = append(*unkFields, b[:n+m]...)
		}
		b = b[n+m:]
	}

	*unkFields = protowire.AppendTag(*unkFields, injectDataFieldID, protowire.BytesType)
...
func ExtractDataFromProto(proto any) (data CustomFields) {
	...
	for len(b) > 0 {
		...
		if num == injectDataFieldID && typ == protowire.BytesType {
