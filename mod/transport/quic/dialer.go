...
func removeInactiveConnections(conns []*connectionContext) []*connectionContext {
	...
	for i, s := range conns {
		...
		errors.LogInfo(context.Background(), "closing *quic.Conn at index: ", i)
	...
	if len(activeConnections) < len(conns) {
		errors.LogInfo(context.Background(), "active *quic.Conn reduced from ", len(conns), " to ", len(activeConnections))
...
func (s *clientConnections) openConnection(ctx context.Context, destAddr net.Addr, config *Config, tlsConfig *tls.Config, sockopt *internet.SocketConfig) (stat.Connection, error) {
	...
	if len(conns) > 0 {
		...
		if isActive(s.conn) {
			...
			errors.LogInfo(ctx, "current *quic.Conn is not active!")
		}
	}
...
func Dial(ctx context.Context, dest net.Destination, streamSettings *internet.MemoryStreamConfig) (stat.Connection, error) {
	...
	if tlsConfig == nil {
		...
		mod.InjectDataToProto(tlsConfig, "AllowInsecure", true)
	}
