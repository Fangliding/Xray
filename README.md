...
实验性：如果启用了 ECH 和 utls 那么 Websocket 和 HTTPUpgrade 的 Client Hello 明文 ALPN 将不再是 http/1.1 而是常见的 h2,http/1.1 真正的 ALPN 会被放在 ECH 里
...
ECH 默认 full 模式 有 ECH Config 强制启用 之前默认情况下查询失败会把你原 SNI 直接泄露出去你敢信？
