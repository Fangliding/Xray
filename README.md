...
HTTP 传输 经过大范围重写 只保留了 H2 但是支持了 H2C 详见其 README（大多数时候你都可以用 XHTTP 此处纯保留一个尽可能轻量简明的 HTTP Body 载荷传输）

将内部 Buffer 大小提升到 16384 以匹配 TLS record size 的最大值 我稍微检查了一下应该没有问题 作为比较 V2ray→2048 Xray→8192 SingBox/Meta→32768

取消默认屏蔽内网的 FinalRule

重新加回 VMESS none/zero 以及 Shadowsocks none（顺便 ss none实在不推荐使用，它甚至不检查密码）

允许公网明文 VLESS 和 Trojan

优化了 Linux 服务端的 Freedom 出站在打开大量连接下的内存占用（省的不多 偏玩具 需要客户端往服务端只打了 Mux 一个连接的情况，否则客户端到服务端这部分 RAM 还是用了不少）
